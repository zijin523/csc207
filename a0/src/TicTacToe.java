...
public class TicTacToe {
	...
	private boolean checkWinDiag() {
		int leftResult = 1;
		int leftDiag = this.board[0][0];
		if(leftDiag!=0) {
			for(int i = 1, j = 1; i < this.board.length && j < this.board.length;) {
				if(this.board[i][j] == leftDiag) {
					i++;
					j++;
				} else {
					leftResult = 0;
		...
		if(leftResult == 1) {
			...
			int rightResult = 1;
			int rightDiag = this.board[0][this.board.length - 1];
			if(rightDiag != 0) {
				for(int i = 1, j = this.board.length - 2; i < this.board.length && 0 <= j;) {
					if(this.board[i][j] == rightDiag) {
						i++;
						j--;
					} else {
						rightResult = 0;
